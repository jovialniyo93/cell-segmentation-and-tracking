...
def evaluate_DET_SEG_TRA(bright_field,period,track_result_path,isWin=False):
    ...
    print("==={}===FN:{}  len(track_list):{}  FN_per_img:{}======".format(period,FN,len(track_list),FN_per_img))
...
def record_performance(excel_path,DET,SEG,TRA,precision,recall,F_measure,FN_per_img,FP_per_img):
    ...
    for i in range(iteration_times):
        ...
        for k in range(3):
            ...
            SEG_sheet.write(k+1,i+1,SEG[i*3+k])
...
def verify_and_evaluate(verify_bright_field_num,ckpt_path,result_dir):
    ...
    for folder in result_path_list:
        ...
        for i in range(1, 4):
            DET, SEG, TRA, pre, cal, fm, fnpi, fppi = evaluate_DET_SEG_TRA(bright_field=verify_bright_field_num, period=i, track_result_path=track_path)
...
def iteration_and_evaluate(bright_field_num,iteration_times,keep_training,verify_BF_num,batch_size=32,shuffle=False):
    ...
    mask_path=str(bright_field_num)+"-GT/human-mask"

    iteration_path=str(bright_field_num)+"-Supervised"
